...
public final class Board extends Observable {
  ...
  private void clearLines() {
    int linecount = 0; //counts cleared lines, used for the point system
    for (int i = my_blocks.size() - 1; i >= 0; i--) {
      ...
      if (clear) {
        my_blocks.remove(i);
        linecount++; //counts cleared lines, used for the point system
      }
    }
    setChanged();
    notifyObservers(linecount); //counts cleared lines, used for the point system
  }
